...
class CBox3 extends Canvas implements Runnable {
	class CBox3PaintListener implements PaintListener {
	...
	public CBox3(Composite parent, int pause) {
		super(parent, SWT.NONE);
		this.pause = pause;
		addPaintListener(new CBox3PaintListener());
	}
	public void run() {
		try {
...
public class Cw42 implements SWTApplication {
	private int grid = 12;
	...
	public void createContents(Composite parent) {
		...
		for(int i = 0; i < (grid * grid); i++) {
			final CBox3 cb = new CBox3(parent, pause);
