...
public class Cw10LiczbyWampirze {
	public static void main(String[]args){
		int[]vamp = new int[3216];
		int licznik = 0;
		String[]vampStr = new String[vamp.length];
		char[][]rozbicie = new char[vampStr.length][4];
		
		//Stworzenie tablicy mo¿liwych liczb wampirzych
		for(int i=0; i<100; i++){
			for(int j=i; j<100; j++){
				if((i*j>1000)&&((i*j)%100!=0)){
					vamp[licznik] = i*j;
					//System.out.println(i*j+", "+licznik);
					licznik++;
				}
			}
		}
